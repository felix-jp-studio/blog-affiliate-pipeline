...
│   └── visual/
│       ├── fixtures/
│       │   └── pages.ts              # 対象 URL・locator・mask 定義
│       ├── helpers/
│       │   ├── stabilize-page.ts     # font 固定, animation 無効, 日付 mask
│       │   └── normalize-text.ts     # whitespace / 連続空白正規化
│       ├── contact.visual.spec.ts
│       ├── sim-hub.visual.spec.ts
│       └── article-template.visual.spec.ts
├── tests/visual/__snapshots__/       # text snapshots (*.txt) — Git 管理
└── tests/visual/__screenshots__/     # PNG baselines — Git 管理（CI と同一パス）
```

### 2.1 `playwright.config.ts` 要点

| 設定                                        | 値                                                                                             | 理由                           |
| ------------------------------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------ |
| `testDir`                                   | `tests/visual`                                                                                 | 機能 E2E（Node scripts）と分離 |
| `webServer.command`                         | `npm run build --prefix site && npm run preview --prefix site -- --host 127.0.0.1 --port 4321` | 本番同等静的配信               |
...
| `test:e2e:visual`        | `playwright test tests/visual`                           | visual 専用                    |
| `test:e2e:visual:update` | `playwright test tests/visual --update-snapshots`        | baseline 更新（ローカル）      |
| `test:e2e:all`           | `npm run test:e2e && npm run test:e2e:visual`            | ローカル全量（任意）           |

CI では `validate` job 内に visual job を **別 job** として追加し、`test:e2e` とは並列実行する（build を共有しない場合は visual job 内で再 build）。
...
| Text   | `.category-hero__title, .category-hero__lead, .category-hub__item h2`（先頭3件） | `sim-hub-head.txt` |
...
| Text   | `.article-hero h1, .article-hero .article-hero__description, .article-body h2`（先頭5） | `article-sim-20gb-head.txt` |
...
| `/sim`                       | `.category-hub__item h2` × 3 | 記事タイトル   | 順序固定（sort 済み）           |
| `/articles/sim-20gb-osusume` | `.article-hero h1`           | 記事タイトル   | trim                            |
| `/articles/sim-20gb-osusume` | `.article-hero__description` | description    | trim                            |
...
## 5. CI 設計（`.github/workflows/ci.yml`）

### 5.1 新規 job: `visual-regression`

```yaml
visual-regression:
  runs-on: ubuntu-latest
  needs: [] # validate と並列可。site build は job 内で実行
  steps:
...
| 項目            | 設定                                                                                                                              |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| トリガ          | `pull_request` + `push` to `main`（`paths-ignore: docs/**` は validate のみ。visual は `site/**` 変更時に paths filter 追加検討） |
| ブラウザ        | Chromium のみ（初回）                                                                                                             |
| 失敗時 artifact | `-expected.png`, `-actual.png`, `-diff.png`, text `-received.txt`                                                                 |
| 所要時間目安    | build 60s + Playwright 30s ≒ 2分                                                                                                  |

### 5.2 paths filter（推奨 Phase 1.1）
...
docs-only PR では visual job をスキップし CI 時間を節約。
...
npm run test:e2e:visual:update   # PNG + txt baseline 更新
...
```mermaid
flowchart LR
  A[PR push] --> B[visual-regression job]
...
| `ci.yml → visual-regression` | PR / push main（site 変更時） | Playwright                                                          |
| `post-deploy-smoke.yml`      | push main（site/**）          | smoke-production + changed-slugs                                    |
| `scheduled-articles.yml`     | cron                          | generator + validate                                                |

Playwright は **dist 検証の後段**ではなく **並列 job**。どちらも `site build` を前提とするため、将来的に `validate` job の build artifact を `visual-regression` に渡して build 時間を共有できる（Phase 1.1 最適化）。
...
| 順序 | タスク                        | 成果物                               | 目安 |
| ---- | ----------------------------- | ------------------------------------ | ---- |
| 1    | Playwright 導入 + config      | `playwright.config.ts`, package.json | 30m  |
| 2    | stabilize / normalize helpers | `tests/visual/helpers/*`             | 30m  |
| 3    | 3 ページ spec + 初回 baseline | `*.visual.spec.ts`, snapshots        | 1.5h |
| 4    | CI visual-regression job      | `.github/workflows/ci.yml`           | 30m  |
| 5    | ドキュメント・npm script 更新 | README / e2e design リンク           | 15m  |
| 6    | changed-slugs 連動（任意）    | dynamic article spec                 | 1h   |

**完了定義**: ローカル `npm run test:e2e:visual` green、CI `visual-regression` green、意図的 CSS 変更で失敗→baseline 更新→再 green を確認。
