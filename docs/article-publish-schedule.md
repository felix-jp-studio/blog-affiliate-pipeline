...
| 公開フロー             | 生成 → PR 作成 → **CI 通過後 auto-merge**（`articles-auto-merge.yml`） |
...
- 記事 PR は `article-publish` ラベル付きで作成され、CI 通過後に `articles-auto-merge.yml` が squash merge する（変更ファイルが記事関連のみの場合）。
...
| 2026-07-20 | 日曜 crosssell 追加。週7本、`keywords.sunday.csv`、Hub `/cost`、cron `0 0 * * 0-6`                 |
