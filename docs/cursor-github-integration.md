...
**推奨 2 層構成: ローカル人間 `gh` + GitHub Actions `GITHUB_TOKEN`**。bot PAT（Machine User）は**任意**。

| レイヤー                   | 用途                                                                               | 認証方式                                                            |
| -------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| **Cursor 公式 GitHub App** | Cloud Agents、Bugbot、`@cursor` コメント起動                                       | [cursor.com/dashboard](https://cursor.com/dashboard) → Integrations |
| **ローカル人間 `gh`**      | Cursor Desktop エージェントの `gh pr create` / `gh issue create`（**デフォルト**） | `./scripts/gh-user.sh`（`felix-jp-studio`）                         |
| **GitHub Actions**         | CI 内 PR 作成・自動マージ等                                                        | `GITHUB_TOKEN`（追加設定不要）                                      |
| **Machine User（任意）**   | PR 作成者を bot に分離したい場合のみ                                               | `GITHUB_BOT_TOKEN` + `./scripts/gh-bot.sh`                          |
...
```mermaid
flowchart LR
  subgraph local [ローカル Cursor エージェント]
    A[Bugbot レビュー] --> B["./scripts/gh-user.sh pr create"]
    B --> C["ラベル: cursor-agent"]
  end
  subgraph github [GitHub]
    C --> D[PR on main]
    D --> E[人間レビュー・マージ]
    F["Actions GITHUB_TOKEN"] --> G[CI 自動マージ等]
  end
  subgraph optional [任意・将来]
    H["./scripts/gh-bot.sh"] -.-> C
  end
```

| 操作                  | 推奨コマンド / 認証                | PR 作成者             | 備考                                    |
| --------------------- | ---------------------------------- | --------------------- | --------------------------------------- |
| エージェント PR 作成  | `./scripts/gh-user.sh pr create`   | `felix-jp-studio`     | `cursor-agent` ラベルで識別             |
| 人間レビュー・マージ  | `./scripts/gh-user.sh` または `gh` | —                     | デフォルト keyring                      |
| CI 自動マージ         | `GITHUB_TOKEN`                     | `github-actions[bot]` | 既存 workflow                           |
| Cloud Agents / Bugbot | Cursor 公式 App                    | Cursor App            | Dashboard 連携                          |
| bot 分離（任意）      | `./scripts/gh-bot.sh`              | `felix-jp-studio-bot` | PAT 未設定時は gh-user にフォールバック |
...
**bot PAT（`GITHUB_BOT_TOKEN`）は必須ではない。** ローカル Cursor エージェントは `./scripts/gh-user.sh` で人間アカウント（`felix-jp-studio`）として PR を作成し、`cursor-agent` ラベルでエージェント PR を識別する。

### bot PAT を省略してよい理由

| 観点             | 人間 gh + ラベルで十分な理由                                         |
| ---------------- | -------------------------------------------------------------------- |
| **PR 識別**      | `cursor-agent` ラベルでフィルタ・通知が可能                          |
| **権限**         | `felix-jp-studio` は Org Member として repo への Write 権限を持つ    |
| **CI 分離**      | Actions 内の操作は `GITHUB_TOKEN`（`github-actions[bot]`）で既に分離 |
| **Cloud Agents** | Cursor 公式 App が Cloud 側 PR を担当（bot PAT とは別レイヤー）      |
| **運用コスト**   | PAT 発行・ローテーション・Org メンバー枠の消費が不要                 |
| **セキュリティ** | 管理するトークンが減る（人間 gh keyring + Actions トークンのみ）     |

### bot PAT を使う場合（任意）

PR 作成者を `felix-jp-studio-bot` に分離したい場合のみ Phase 2（下記）を実施。`./scripts/gh-bot.sh` はトークン未設定または API 失敗時に `./scripts/gh-user.sh` へフォールバックする。

---

## 環境構築（推奨: 人間 gh デフォルト）

**目的**: 普段もエージェントも `./scripts/gh-user.sh`（`felix-jp-studio`）を使う。bot PAT は任意。
...
| Cursor エージェント（PR/Issue） | `./scripts/gh-user.sh`（**デフォルト**） | 同上（人間 keyring）                                 |
| bot 分離（任意）                | `./scripts/gh-bot.sh`                    | 環境変数 `GITHUB_BOT_TOKEN`（または `GH_TOKEN_BOT`） |
...
# ブラウザで felix-jp-studio として認証
gh auth status
# → Active account: felix-jp-studio
```

### 2. fine-grained PAT を発行（任意・bot アカウントでログイン）
...
### 3. トークンをローカルに保存（任意・repo にコミットしない）
...
### 4. シェルエイリアス（任意・ユーザーが ~/.zshrc に手動追加）
...
./scripts/gh-user.sh auth status
# → Active account: felix-jp-studio
...
`.cursor/rules/github-bot-account.mdc` — エージェントは PR/Issue 作成に `./scripts/gh-user.sh` をデフォルト使用。`cursor-agent` ラベル付与。
...
| Cursor エージェントが PR | `./scripts/gh-user.sh pr create` |
| bot 分離（任意）         | `./scripts/gh-bot.sh pr create`  |
...
### Phase 2: Machine User（任意・PR 作成者を bot に分離したい場合）

ローカル Cursor エージェントのデフォルトは `./scripts/gh-user.sh`（人間アカウント）。PR/Issue の作成者を bot に分離したい場合のみ Phase 2 を実施する。
...
3. リポジトリルール（`.cursor/rules/pr-review.mdc`）はそのまま有効
   - ローカル Bugbot サブエージェント → `./scripts/gh-user.sh pr create` の流れを維持
...
2. エージェント PR（`cursor-agent` ラベル付き）が作成されると通知

#### 3-2. PR ラベルで識別（推奨）

エージェント PR 作成時に `cursor-agent` ラベルを付与:

```bash
./scripts/gh-user.sh pr create ... --label "cursor-agent"
```
...
| **監査**            | `cursor-agent` ラベルでエージェント PR を識別。bot PAT 利用時は作成者も `felix-jp-studio-bot`      |
...
| `gh pr create` が Permission denied    | `gh auth status` で `felix-jp-studio` が Active か確認。Org/repo の Write 権限を確認        |
...
| エージェント PR を識別したい           | `--label "cursor-agent"` を付与。GitHub Labels でフィルタ                                   |
| bot PAT を使いたいが失敗する           | `./scripts/gh-bot.sh` は自動で `gh-user.sh` にフォールバック。PAT 権限を再確認              |
