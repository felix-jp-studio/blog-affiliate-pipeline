...
const { costHub } = visualPages;

test.describe("cost hub page", () => {
  test("visual and text snapshots", async ({ page }) => {
    await page.goto(costHub.path);
    await stabilizePage(page);

    const hub = page.locator(costHub.visualLocator);
    await expect(hub).toHaveScreenshot(costHub.visualSnapshot, {
    ...
    expect(text).toMatchSnapshot(costHub.textSnapshot);
  });
});
